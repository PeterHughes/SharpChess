// --------------------------------------------------------------------------------------------------------------------
// <copyright file="PlayerDebug.cs" company="SharpChess.com">
//   Peter Hughes
// </copyright>
// <summary>
...
// PlayerDebug program is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
// PlayerDebug program is distributed in the hope that it will be useful,
...
namespace SharpChess
{
    ...
    public static class PlayerDebug
    {
        ...
        /// </remarks>
        private static bool DebugMatchLine(string strVariation, int iPly, Move moveThis, int intSearchDepth, int intMaxSearchDepth)
        {
            const int iSAN_LENGTH = 5; // Length of Abbreviation of the piece + From square + To square
            if (PlayerDebug.m_iDbgLevel == iPly)
            {
                // Is the level of depth of the variation reached?
                if (PlayerDebug.m_strDbgLine.Length == 0)
                {
                    ...
                    PlayerDebug.m_iDbgIteration = Convert.ToInt32(strVariation.Substring(0, indPos));
                    PlayerDebug.m_strDbgLine = strVariation.Substring(indPos); // Parse the variation
                    PlayerDebug.m_strDbgLine = PlayerDebug.m_strDbgLine.Replace(" ", string.Empty); // removing all whitespaces
                    PlayerDebug.m_strDbgLine = PlayerDebug.m_strDbgLine.Replace("x", string.Empty); // removing all "x"
                }

                if (intSearchDepth == PlayerDebug.m_iDbgIteration)
                {
                    // Number of iteration of AlphaBeta at the top level
                    int indPiece = iPly * iSAN_LENGTH; // Index where begins the notation of the move
                    int iLenVar = PlayerDebug.m_strDbgLine.Length;
                    string strMoveDescr = moveThis.Piece.Abbreviation + moveThis.From.Name + moveThis.To.Name;
                    if ((indPiece <= iLenVar - iSAN_LENGTH)
                        && (strMoveDescr == PlayerDebug.m_strDbgLine.Substring(indPiece, iSAN_LENGTH)))
                    {
                        if (++PlayerDebug.m_iDbgLevel == iLenVar / iSAN_LENGTH)
                        {
        ...
        private static bool DebugMatchVariation(int iPly, Move moveThis, int intSearchDepth, int intMaxSearchDepth)
        {
            ...
            return PlayerDebug.DebugMatchLine("5 Bb3a4 Bc8d7 Ba4xc6 Bd7xc6 Rf1e1 Bf8e7 Bc1d2", iPly, moveThis, intSearchDepth, intMaxSearchDepth);
