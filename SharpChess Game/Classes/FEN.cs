...
namespace SharpChess
{
	...
	public class FEN
	{
		...
		private static void MovePieceToFENPosition(ref char charToken, int intFile, int intRank, bool blnAnyLocation, bool blnAllowPromotion)
		{
			...
			{
				if (
					(pieceCaptured.Name==piecename || blnAllowPromotion && pieceCaptured.Name==Piece.enmName.Pawn )
					&&
		...
		private static bool FenCheck1PiecePlace(string strFen)
		{
			...
			for (int indChar = 0; indChar < strFen.Length; indChar++)
			{
				...
				{
					case 'K': iNbrK++; iNbrSquare++; break; case 'k': iNbrk++; iNbrSquare++; break;
					case 'Q': iNbrQ++; iNbrSquare++; break; case 'q': iNbrq++; iNbrSquare++; break;
					case 'R': iNbrR++; iNbrSquare++; break; case 'r': iNbrr++; iNbrSquare++; break;
					case 'B': iNbrB++; iNbrSquare++; break; case 'b': iNbrb++; iNbrSquare++; break;
					case 'N': iNbrN++; iNbrSquare++; break; case 'n': iNbrn++; iNbrSquare++; break;
					case 'P': iNbrP++; iNbrSquare++; break; case 'p': iNbrp++; iNbrSquare++; break;
					case '/': iNbrSlash++;
						if (iNbrSquare != 8)
							throw new ValidationException("1010: " + FenHlpMsg(1) +
		...
		private static bool FenCheck3Castle(string strFen)
		{
			...
			for (int indChar = 0; indChar < strFen.Length; indChar++)
			{
				switch (strFen[indChar])
				{
					case 'K': iNbrK++; break; case 'k': iNbrk++; break;
					case 'Q': iNbrQ++; break; case 'q': iNbrq++; break;
					case '-': iNbrDash++; break;
					default:
		...
		private static bool FenCheck5Counter50MoveDraw(string strFen)
		{
			...
			catch {throw new ValidationException("1510: " + FenHlpMsg(5) + "Expect a half move number for the rule of 50 moves");}
		...
		private static bool FenCheck6NbrMove(string strFen)
		{
			...
			catch {throw new ValidationException("1600: " + FenHlpMsg(6) + "Expect a full move number");}
		...
		public static string GetBoardPosition()
		{
			...
			System.Text.StringBuilder strbFen = new System.Text.StringBuilder();
		...
		private static void FenGet1Pieces(System.Text.StringBuilder strbFen)
		{
			for(int iNbrEmptySquare = 0, indRank = Board.RANK_COUNT - 1 ; indRank >= 0; indRank--)
			{
				...
				for(int indFile = 0; indFile < Board.FILE_COUNT; indFile++) // Browse by column
				{
		...
		private static bool FenGet3CastlingFuture(Piece pieceK, System.Text.StringBuilder strbFen)
		{
