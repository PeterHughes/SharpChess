...
{
	/// <summary>
	/// Summary description for WinBoard.
	/// </summary>
	public class WinBoard
	{
		...
		public delegate void delegatetypeCommunicationEvent(string strMessage);
		public delegate void delegatetypeStandardEvent();

		public static event delegatetypeCommunicationEvent WinBoardInputEvent;
		public static event delegatetypeCommunicationEvent WinBoardOutputEvent;
		public static event delegatetypeStandardEvent WinBoardQuitEvent;
		public static event delegatetypeStandardEvent WinBoardTimeUpdatedEvent;
		
		private static Thread m_threadWinBoardListener = null;
		private static bool m_blnActive = false;

		public static Thread threadListener
		{
			get { return m_threadWinBoardListener; }
		}

        public static bool Active
        {
            get { return m_blnActive; }
        }
        ...
        public static void DetermineStatus()
        {
            ...
            m_blnActive = (strMessage != null && strMessage.StartsWith("xboard"));
            if (m_blnActive)
            {
                WinBoard.ShowGUI = strMessage.Contains("showgui");
        ...
        public static void StartListener()
		{
			if (m_blnActive)
			{
				m_threadWinBoardListener = new Thread(new ThreadStart(Listen));
				m_threadWinBoardListener.Priority = System.Threading.ThreadPriority.Normal;
				m_threadWinBoardListener.Start();
			}
		}
		...
		public static void StopListener()
		{
			if (m_blnActive)
			{
				m_threadWinBoardListener.Abort();
				m_threadWinBoardListener.Join();
				m_threadWinBoardListener = null;
			}
		}

        public static void SendMove(SharpChess.Move move)
        {
            if (m_blnActive)
            {
                WinBoard.SendOutputMessage("move " + move.From.Name + move.To.Name);
            }
        }
        ...
        public static void SendMoveTime(TimeSpan timeSpan)
        {
            if (m_blnActive)
            {
                WinBoard.SendOutputMessage("movetime " + timeSpan.ToString());
            }
        }
        ...
        public static void SendCheckMate()
		{
			if (m_blnActive)
			{
				WinBoard.SendOutputMessage((Game.PlayerBlack.IsInCheckMate ? "1-0 {White mates}" : "0-1 {Black mates}") );
			}
		}
		...
		public static void SendCheckStaleMate()
		{
			if (m_blnActive)
			{
				WinBoard.SendOutputMessage("1/2-1/2 {Stalemate}" );
			}
		}
		...
		public static void SendDrawByRepetition()
		{
			if (m_blnActive)
			{
				WinBoard.SendOutputMessage("1/2-1/2 {Draw by repetition}" );
			}
		}
		...
		public static void SendDrawByFiftyMoveRule()
		{
			if (m_blnActive)
			{
				WinBoard.SendOutputMessage("1/2-1/2 {Draw by 50 move rule}" );
			}
		}
		...
		public static void SendDrawByInsufficientMaterial()
		{
			if (m_blnActive)
			{
				WinBoard.SendOutputMessage("1/2-1/2 {Draw by insufficient material}" );
			}
		}
		...
		public static void SendThinking(int Ply, int Score, TimeSpan ThinkingTime, int Nodes, string PrinicalVariation)
		{
			...
			*/
			if (m_blnActive && Game.ShowThinking)
			{
				int intScoreInCentipawns = Score/10;
				int intTimeIncentiseconds = Convert.ToInt32(ThinkingTime.TotalMilliseconds/100);
				WinBoard.SendOutputMessage( 
											Ply.ToString() 
											+ " " + intScoreInCentipawns.ToString()
											+ " " + intTimeIncentiseconds.ToString()
											+ " " + Nodes.ToString()
											+ " " + PrinicalVariation
										  );
			}
		...
		public static void SendAnalyzeStatus(TimeSpan ThinkingTime, int Nodes, int Ply, int MovesRemaining, int TotalMoves, Move moveCurrent)
		{
			/*
			...
			*/
			if (m_blnActive && Game.IsInAnalyseMode)
			{
				int intTimeIncentiseconds = Convert.ToInt32(ThinkingTime.TotalMilliseconds/100);
				WinBoard.SendOutputMessage( 
					"stat01:"
					...
					+ " " + TotalMoves.ToString()
					+ " " + moveCurrent.From.Name + moveCurrent.To.Name
					);
			}
		}
		...
		public static void ProcessInputEvent(string strMessage)
		{
			try
			{
				...
				else if (strMessage.StartsWith("protover "))
				{
					...
					*/
					...
					strFeatures += " pause=1";
					strFeatures += " done=1";
					WinBoard.SendOutputMessage("feature" + strFeatures);
				}
                ...
                else if (strMessage.StartsWith("load "))
                {
                    ...
                    {
                        WinBoard.SendOutputMessage("Loaded save game: " + strPath);
                    }
                    else
                    {
				...
				else if (strMessage.StartsWith("time "))
				{
					...
					*/
					...
					{
						Game.PlayerToPlay.OtherPlayer.Clock.TimeElapsed = ( new TimeSpan(Game.ClockFixedTimePerMove.Ticks*Game.MoveNo)) - (new TimeSpan(long.Parse(strMessage.Substring("time ".Length))*100000));
					}
					else if (Game.ClockIncrementPerMove.Ticks > 0)
					{
						Game.PlayerToPlay.OtherPlayer.Clock.TimeElapsed = ( new TimeSpan(Game.ClockTime.Ticks + Game.ClockIncrementPerMove.Ticks*Game.MoveNo)) - (new TimeSpan(long.Parse(strMessage.Substring("time ".Length))*100000));
					}
					else
					{
						Game.PlayerToPlay.OtherPlayer.Clock.TimeElapsed = (new TimeSpan(Game.ClockTime.Ticks * Game.PlayerToPlay.OtherPlayer.Clock.ControlPeriod)) - (new TimeSpan(long.Parse(strMessage.Substring("time ".Length))*100000));
					}
					WinBoardTimeUpdatedEvent();
				}
				else if (strMessage.StartsWith("otim "))
				{
					...
					*/
					if (Game.ClockFixedTimePerMove.Ticks > 0)
					{
						Game.PlayerToPlay.Clock.TimeElapsed = ( new TimeSpan(Game.ClockFixedTimePerMove.Ticks*Game.MoveNo)) - (new TimeSpan(long.Parse(strMessage.Substring("time ".Length))*100000));
					}
					else if (Game.ClockIncrementPerMove.Ticks > 0)
					{
					...
					{
						Game.PlayerToPlay.Clock.TimeElapsed = (new TimeSpan(Game.ClockTime.Ticks * Game.PlayerToPlay.Clock.ControlPeriod)) - (new TimeSpan(long.Parse(strMessage.Substring("time ".Length))*100000));
					}
					WinBoardTimeUpdatedEvent();
				}
				...
				else if (strMessage.StartsWith("ping "))
				{
					...
					*/
					...
					WinBoard.SendOutputMessage("pong " + strMessage.Substring(5));
				}
				...
				else if (strMessage.StartsWith("setboard "))
				{
					...
					*/
					...
					{
						WinBoard.SendOutputMessage("tellusererror Illegal position: " + x.FENMessage);
					}
				}
				...
				else if (strMessage==".")
				{
					...
					if (Game.PlayerToPlay.IsThinking)
					{
						SendAnalyzeStatus(Game.PlayerToPlay.ThinkingTimeElpased, Game.PlayerToPlay.PositionsSearched, Game.PlayerToPlay.SearchDepth, Game.PlayerToPlay.TotalPositionsToSearch - Game.PlayerToPlay.SearchPositionNo, Game.PlayerToPlay.TotalPositionsToSearch, Game.PlayerToPlay.CurrentMove);
					}
				}
			...
			{
				WinBoard.SendOutputMessage(e.WinBoardMessage);
			}
		}
